...
# Create a class that will give us an object that we caan use to connect to a database
        ...
        with self.connection.cursor() as cursor:
            try:
                ...
                if query.lower().find("insert") >= 0:
                    # if the query is an insert, return the id of the last row, since that is the row we 
                    # just added
                    self.connection.commit()
                    return cursor.lastrowid
                elif query.lower().find("select") >= 0:
                    # if the query is a select, return everything that is fetched from the database
                    # the result iwll be a list of dictionaries
                    result = cursor.fetchall()
                    return result
                else:
                    # if the query is not an insert or a select, such as an update or delete, commit the
                    # changes, return nothing
                    self.connection.commit()
